# Archived non-positional-encoding trainer. Only the default output path was sanitized.
# Public paper configurations are selected by paper_training.py.
import os
import sys
import numpy as np
import torch
...
class SignalTransformerModel(nn.Module):
    ...
    def __init__(self, dim=128, num_classes=6, dropout=None):
        ...
        self.embed_g = PatchEmbedding(embed_in_ch, dim, patch=16)
        self.embed_a = PatchEmbedding(embed_in_ch, dim, patch=16)
        self.embed_v = PatchEmbedding(embed_in_ch, dim, patch=32)
